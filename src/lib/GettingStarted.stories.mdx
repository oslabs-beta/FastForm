...
* The first name field was left blank, which triggered an error for the ['required'](?path=/story/validators-examples--required) validator. 
* The first name field did not match our other name field with name 'John Doe', triggering an error for the ['mustMatch'](?path=/story/validators-examples--must-match) validator. 
* No options were checked for the ice cream field, triggering an error for the ['minNumOptions'](?path=/story/validators-examples--min-num-options) validator. 

Once each validator's conditions were met, the errors were cleared. 

In the code snippet below, our [Field](?path=/story/field-field--page) components for both names and icecream are within our [FastForm](?path=/story/fastform-example--page) component. FastForm takes in initial values, a handleSubmit function, and validator functions. 
